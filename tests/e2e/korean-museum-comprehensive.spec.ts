...
test.describe('Korean Museum App - Comprehensive Test Suite', () => {
  ...
  test('1. Homepage loads correctly with Korean content', async ({ page }) => {
    ...
    // The app redirects to /ko by default
    const url = page.url();
    expect(url).toMatch(/\/ko|localhost:3000/);
    
    // Check for Korean title
    const title = await page.title();
    expect(title).toMatch(/국립중앙박물관|Korean|Museum/);
  ...
  test('3. Language switcher functionality', async ({ page }) => {
    ...
    // Find language button (globe icon or language text)
    ...
    if (await langButton.isVisible()) {
      ...
      if (await enOption.isVisible()) {
        ...
        const englishHeader = await page.locator('h1:has-text("National Museum")').count();
        expect(englishHeader).toBeGreaterThan(0);
        ...
        await langButton.click();
        await page.waitForTimeout(500);
        const koOption = page.locator('text=한국어, text=KO').first();
  ...
  test('5. Image loading with blur placeholders', async ({ page }) => {
    ...
    const images = page.locator('img[data-nimg], img[loading="lazy"]');
    ...
    if (blurFound) {
      ...
      console.log(`⚠ ${imageCount} images found but no obvious blur placeholders`);
    }
  });
  ...
  test('6. Favorites functionality', async ({ page }) => {
    ...
    const favoriteBtn = page.locator('button').filter({
      has: page.locator('svg path[d*="M3.172"], svg path[d*="heart"], text=♥')
    }).first();
    ...
    if (await favoriteBtn.isVisible()) {
      ...
      // Check state changed
      const afterClass = await favoriteBtn.getAttribute('class') || '';
  ...
  test('7. Hall statistics display', async ({ page }) => {
    ...
    // Check hall cards on homepage
    const hallStats = page.locator('text=/국보|보물|[0-9]+점/');
  ...
  test('9. Error handling', async ({ page }) => {
    ...
    const hasError = await page.locator('text=/404|찾을 수 없|오류/').count() > 0;
    ...
    if (hasError) {
      ...
      console.log('⚠ No clear error handling');
    }
  });
  ...
  test('11. Responsive design', async ({ page }) => {
