...
export default function ArtifactDetailClient({ artifact, relatedArtifacts }: ArtifactDetailClientProps) {
  ...
  const baseImage = artifact.image || `https://via.placeholder.com/600x600?text=${encodeURIComponent(artifact.name[language as keyof typeof artifact.name])}`
  
  const images = [
    baseImage,
    // 추가 이미지가 없으면 placeholder만 표시
    ...(artifact.image ? [] : [
      `https://via.placeholder.com/600x600?text=${encodeURIComponent(artifact.name[language as keyof typeof artifact.name])}+측면`,
      `https://via.placeholder.com/600x600?text=${encodeURIComponent(artifact.name[language as keyof typeof artifact.name])}+세부`,
    ])
  ]
