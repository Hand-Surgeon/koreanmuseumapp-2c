...
export default function Home() {
  ...
  const filteredArtifacts = useMemo(() => {
    ...
    return filterArtifacts(artifacts, searchTerm, t.all, language as 'ko' | 'en' | 'zh' | 'ja' | 'th', t.all)
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <Badge variant="secondary">{t.masterpieces100}</Badge>
            </div>
            <LanguageSelector />
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {!searchTerm && (
          <>
            ...
            {/* Halls Section */}
            <section>
              <h2 className="text-2xl font-bold text-gray-900 mb-6">{language === 'ko' ? '전시관' : 'Exhibition Halls'}</h2>
                    ...
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <h3 className="font-semibold text-lg">{t[config.translatedName]}</h3>
                            <p className="text-sm text-gray-500">{key}</p>
                          </div>
                        </div>
                        <Badge variant="secondary">{artifacts.filter(a => a.hall === key).length}</Badge>
                      </div>
              ...
              {selectedHall && (
                <div className="mt-8">
                  <h3 className="text-xl font-semibold mb-4">
                    {t[hallConfigs[selectedHall as keyof typeof hallConfigs].translatedName]} {t.artifactList}
                  </h3>
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
