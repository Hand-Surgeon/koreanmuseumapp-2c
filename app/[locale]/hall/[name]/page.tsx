...
import { getHallName } from "@/lib/hall-utils"
import { getImageProps } from "@/lib/image-config"
import { VirtualArtifactList } from "@/lib/dynamic-imports"
...
function HallPageClient({ name }: { name: string }) {
  ...
  const hallName = getHallName(decodeURIComponent(name))
  ...
  const filteredArtifacts = hallArtifacts.filter((artifact) => {
    const matchesSearch =
      artifact.name[language as keyof typeof artifact.name].toLowerCase().includes(searchTerm.toLowerCase()) ||
      artifact.description[language as keyof typeof artifact.description].toLowerCase().includes(searchTerm.toLowerCase()) ||
      artifact.period[language as keyof typeof artifact.period].toLowerCase().includes(searchTerm.toLowerCase())
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="px-4 py-6">
                    ...
                    <CardContent className="p-0">
                      <div className="relative">
                        <AspectRatio ratio={16 / 9}>
                          <Image
                            {...getImageProps(artifact.image || '', 'main', 'detail', true)}
                      ...
                      <div className="p-4">
                        <div className="flex items-center justify-between mb-2">
                          <h3 className="font-semibold text-gray-900 text-lg">{artifact.name[language as keyof typeof artifact.name]}</h3>
                        ...
                        <p className="text-sm text-gray-700 line-clamp-2 mb-3">{artifact.description[language as keyof typeof artifact.description]}</p>
        ...
        {/* Regular Artifacts with Virtual Scrolling */}
        {regularArtifacts.length > 0 && (
          <div>
            <h2 className="text-xl font-semibold text-gray-900 mb-4">
              {featuredArtifacts.length > 0 ? t.moreArtifacts : t.artifactList}
            </h2>
            {/* Use virtual scrolling for better performance with many items */}
            {regularArtifacts.length > 20 ? (
              <VirtualArtifactList 
                artifacts={regularArtifacts} 
                columns={2}
                basePath="../"
              />
            ) : (
                      ...
                      <CardContent className="p-0">
                        <div className="relative">
                          <AspectRatio ratio={4 / 3}>
                            <Image
                              {...getImageProps(artifact.image || '', 'main', 'card', true)}
                              alt={artifact.name[language as keyof typeof artifact.name]}
                              fill
                              className="object-contain bg-gray-50 group-hover:scale-105 transition-transform duration-300"
                        ...
                        <div className="p-3">
                          ...
                          <p className="text-xs text-gray-600 mb-2">{artifact.period[language as keyof typeof artifact.period]}</p>
                          <p className="text-xs text-gray-700 line-clamp-2 mb-2">{artifact.description[language as keyof typeof artifact.description]}</p>
                          <Badge variant="secondary" className="rounded-full text-xs">
                            {artifact.category}
                          </Badge>
                        </div>
