...
export default function FavoritesPage() {
  ...
  const filteredArtifacts = useMemo(() => {
    ...
    return filterArtifacts(favoriteArtifacts, searchTerm, t.all, language as 'ko' | 'en' | 'zh' | 'ja' | 'th', t.all)
  ...
  const handleExport = () => {
    const data = favoriteArtifacts.map(artifact => ({
      id: artifact.id,
      name: artifact.name[language as keyof typeof artifact.name],
      period: artifact.period[language as keyof typeof artifact.period],
  ...
  const handleShare = async () => {
    const shareText = favoriteArtifacts
      .map(artifact => `• ${artifact.name[language as keyof typeof artifact.name]}`)
      .join('\n')
        ...
        {favoritesCount === 0 ? (
          <Card className="p-12 text-center">
            <CardContent>
              ...
              <Link href="../">
                <Button>{t.browseArtifacts}</Button>
              </Link>
            </CardContent>
          </Card>
        ) : (
